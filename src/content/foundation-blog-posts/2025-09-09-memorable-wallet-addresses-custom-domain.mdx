...
![Illustration showing turning long wallet addresses to custom domain payment pointers](/img/foundation-blog/2025-09-09/memorable-wallet-addresses-on-own-domain.png)
...
![Screenshot showing Cloudflare dashboard with the Page rule editor showing setup of my domain using a redirect to my GateHub wallet](/img/foundation-blog/2025-09-09/cloudflare-page-rule.png)
...
![Screenshot of Cloudflare dashboard with a transform rule to support CORS for our wallet address](/img/foundation-blog/2025-09-09/cloudflare-transform-rule.png)
...
<CodeBlock language="nginx" code={`# nginx.conf
...
<CodeBlock language="javascript" code={`// index.js
  const WALLET_ADDRESS = 'https://ilp.wallet.com/abc/xyz'
...
<CodeBlock language="bash" code={`# for example, with curl
...
<CodeBlock language="html" code={`<link rel="monetization" href="/.well-known/pay" />`} />
...
<CodeBlock language="webidl" code={`interface MonetizationEvent : Event {
...
<CodeBlock language="typescript" code={`window.addEventListener('monetization', (event) => {
