...
<CodeBlock
  language="typescript"
  code={`async function getPopup(context: BrowserContext, popupUrl: string) {
...
<CodeBlock
  language="typescript"
  code={`// tests/e2e/fixtures/base.ts
...
<CodeBlock
  language="typescript"
  code={`// real tests are of course more complex
...
<CodeBlock
  language="typescript"
  code={`context.on('requestfinished', async function intercept(req) {
