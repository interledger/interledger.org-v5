...
date: 2024-12-03
pathSlug: e2e-testing-wm-browser-extension
featured: false
locale: en
articleBios:
  - author: Sid Vishnoi
...
![Playwright UI while testing the Web Monetization browser extension](/img/foundation-blog/2024-12-03/playwright.png)
...
<CodeBlock language="typescript" code={`function loadExtension(pathToExtension: string): Promise<BrowserContext> {
...
<CodeBlock language="typescript" code={`let background = context.serviceWorkers()[0]
if (!background) {
...
<CodeBlock language="typescript" code={`const storageData = await background.evaluate(() => {
...
<CodeBlock language="typescript" code={`const data = await background.evaluate(() => {
...
<CodeBlock language="typescript" code={`const { connected } = await background.evaluate(() => {
...
![Web Monetization extension's popup](/img/foundation-blog/2024-12-03/extension-popup.png)
...
<CodeBlock language="typescript" code={`const popup = await context.newPage()
...
<CodeBlock language="typescript" code={`async function getPopup(context: BrowserContext, popupUrl: string) {
...
<CodeBlock language="typescript" code={`// tests/e2e/fixtures/base.ts
...
<CodeBlock language="typescript" code={`// real tests are of course more complex
...
<CodeBlock language="typescript" code={`context.on('requestfinished', async function intercept(req) {
...
<CodeBlock language="typescript" code={`// tests/e2e/auth.setup.ts
