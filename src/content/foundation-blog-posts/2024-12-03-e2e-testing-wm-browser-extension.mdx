...
<CodeBlock
  language="typescript"
  code={`function loadExtension(pathToExtension: string): Promise<BrowserContext> {
...
<CodeBlock
  language="typescript"
  code={`let background = context.serviceWorkers()[0]
if (!background) {
...
<CodeBlock
  language="typescript"
  code={`const storageData = await background.evaluate(() => {
...
<CodeBlock
  language="typescript"
  code={`const data = await background.evaluate(() => {
...
<CodeBlock
  language="typescript"
  code={`const { connected } = await background.evaluate(() => {
...
<CodeBlock
  language="typescript"
  code={`const popup = await context.newPage()
...
const { connected } = await background.evaluate(() => {/* chrome.storage... */})
