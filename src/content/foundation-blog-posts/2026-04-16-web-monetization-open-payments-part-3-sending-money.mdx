...
relatedArticles:
  - web-monetization-open-payments-part-1-connecting-wallet
  - web-monetization-open-payments-part-2-payment-sessions
...
In [Part 1](https://interledger.org/blog/web-monetization-open-payments-part-1-connecting-wallet/) of the series, we covered how to link your wallet to the extension and secure the access tokens required for future transactions. In [Part 2](https://interledger.org/blog/web-monetization-open-payments-part-2-payment-sessions), we explored the receiving side: identifying a website's receiving wallet addresses via `<link rel="monetization">` and calculating the amounts our wallet can pay into them. Now, we'll combine these elements to actually move money.
...
<CodeBlock language="typescript" code={`const MS_IN_HOUR = 60 * 60 * 1000
...
<CodeBlock language="typescript" code={`// Note: this is a simplified version
...
[Interactive demo: how the payment bucket fills](https://labs.sidvishnoi.com/web-monetization/open-payments-part-3-sending-money/payment-bucket/)
...
[Interactive demo: sequence of payments with iframes](https://labs.sidvishnoi.com/web-monetization/open-payments-part-3-sending-money/iframes-flow)
...
<CodeBlock language="javascript" code={`const outgoingPayment = await openPaymentsClient.outgoingPayment.create(
  {
...
<CodeBlock language="typescript" code={`if (document.createElement('link').relList.supports('monetization')) {
...
<CodeBlock language="javascript" code={`window.addEventListener('monetization', (event) => {
