...
export async function buildMap(): Promise<Record<string, TranslationEntry>> {
  ...
  // Foundation profiles can have pathSlugs that start with a URL prefix that
  // matches another route base (e.g. 'grant/fellowship/andria-barrett' where
  // '/grant' is also the grant-pages base). routeContextFromPathname strips
  // that prefix, so the switcher looks up 'fellowship/andria-barrett' — not the
  // full slug. Add alias entries under the stripped key so the lookup succeeds.
