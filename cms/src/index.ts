...
export default {
   ...
   */
  async bootstrap({ strapi }: { strapi: StrapiInstance & { server?: { use?: (middleware: unknown) => void } } }) {
    ...
    const { validateNoNestedJsx } = require('./utils/contentValidation')
    const CONTENT_MANAGER_PATTERN = /^\/content-manager\/collection-types\/api::/
    strapi.server?.use?.(async (ctx: { method?: string; url?: string; request?: { body?: { content?: unknown } }; status?: number; body?: unknown }, next: () => Promise<void>) => {
      if (
