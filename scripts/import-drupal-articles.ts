...
async function fetchMedia(imageAPILink) {
  try {
    ...
    const imageUrl = drupalMediaUrl + imageName
...
async function getMediaInfo(media) {
  ...
  return await fetchMedia(imageAPILink)
}
...
async function importArticlesFromDrupal() {
  ...
  // filtered article with multiple photos inside + author
  let articleUrl =
    'https://staging.interledger.org/jsonapi/node/article?filter[title]=Shanghai,%20Flip%20Phones,%20and%20Robots:%20An%20Interledger%20Engineering%20Adventure'
